...
class TrendDetector(bt.Indicator):
    ...
    def next(self):
        ...
        is_bullish = (
                ...
                plus_di_value > minus_di_value
        )

        is_bearish = (
                (price_below_mid or price_near_bottom) and
                minus_di_value > plus_di_value
        )
        ...
        if self.is_daily:
            ...
            print(f"  is_bullish: {is_bullish}, is_bearish: {is_bearish}, di_diff_ratio: {format_num(di_diff_ratio)}")
            print(f"  adx_near_threshold: {adx_near_threshold}, adx_far_below_threshold: {adx_far_below_threshold}")
            print(
                f"  is_narrow_channel: {is_narrow_channel}, is_boll_mid_rising: {is_boll_mid_rising}, is_boll_mid_falling: {is_boll_mid_falling}")
            print(
                f"  DMI指标: period={self.params.dmi_period},threshold={self.params.adx_threshold}, ADX={format_num(adx_value)}, +DI={format_num(plus_di_value)}, -DI={format_num(minus_di_value)}")
            print(
                f"  BOLL指标:period={self.params.boll_period}, boll_dev={self.params.boll_dev} 上轨={format_num(boll_top)}, 中轨={format_num(boll_mid)}, 下轨={format_num(boll_bot)}")
